...
//  bat 'rmdir /s /q 3rd-party'
  ...
  withEnv(['config=Release']) {
    ...
    bat 'call "C:\\Program Files (x86)\\Embarcadero\\Studio\\14.0\\bin\\rsvars.bat" && msbuild /t:build /p:config=%config%;Platform=Win32;DCC_DebugInfoInExe=true MyBasicPackage.dproj && msbuild /t:build /p:config=%config%;Platform=Win32;DCC_DebugInfoInExe=true MyPackageTester.dproj && msbuild /t:build /p:config=%config%;Platform=Win32;DCC_DebugInfoInExe=true MyBasicApp.dproj'
